  ...
  return (
    <div className={`max-w-4xl mx-auto py-12 px-4 ${locale === 'ar' ? 'font-cairo' : ''}`}>
      <div className="relative">
        ...
        <div className={`absolute ${locale === 'ar' ? 'right-[20px]' : 'left-[20px]'} top-6 bottom-6 w-0.5 border-l-2 border-dashed border-gray-300`} />
        
        {/* Award nodes */}
        {awards.map((award, index) => (
          <div key={index} className={`flex items-start mb-12 relative ${locale === 'ar' ? 'flex-row-reverse' : ''}`}>
            {/* Node circle */}
            <div className={`absolute ${locale === 'ar' ? 'right-[20px]' : 'left-[20px]'} -translate-x-1/2 rounded-full flex items-center justify-center bg-dark border-2 w-[50px] h-[50px]`} >
              <Image src={medal} alt={'medal'} className="" width={20} height={20} />
            </div>
            
            {/* Award content */}
            <div className={`flex-1 ${locale === 'ar' ? 'mr-[calc(20px+2rem)]' : 'ml-[calc(20px+2rem)]'}`}>
